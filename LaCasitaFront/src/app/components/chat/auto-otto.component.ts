...
export class AutoOTTOChatComponent implements OnInit {
  ...
  accelerationincludinggravityx1 = 0;
  accelerationincludinggravityy1 = 0;
  accelerationincludinggravityz1 = 0;
...
this.listener1 = this.renderer1.listen( window , 'devicemotion', (event) => {
        ...
        this.alpha1 = Math.round(event.acceleration.x);
        this.beta1 = Math.round(event.acceleration.y);
        this.gamma1 = Math.round(event.acceleration.z);
...
this.autoOTTOSubscription = this._chatService.getMessagesAuto()
         .subscribe( (pay: any) => {
           const de: string = pay.de;
           const cuerpo: string = pay.cuerpo;
...
this.autoOTTOLOGSubscription = this._chatService.getMessagesAutoLOG()
         .subscribe( (msg: any) => {
           const de: string = msg.nombre;
           const beta: string = msg.bet;
           const gamma: string = msg.gamm;
           const motor: string = msg.MOTOR;
